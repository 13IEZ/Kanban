	...
	switch (action.type) {
		case FETCH_STATUS_SUCCESS:
			return {
				...
				readyTasks: action.payload.filter(elem => elem.status === 'ready').sort((a,b) => a.counter > b.counter ? 1 : -1),
				inProgressTasks: action.payload.filter(elem => elem.status === 'in progress').sort((a,b) => a.counter > b.counter ? 1 : -1),
				finishedTasks: action.payload.filter(elem => elem.status === 'finished').sort((a,b) => a.counter > b.counter ? 1 : -1)
			};
